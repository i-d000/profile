...
for ($i = 0; $i < count($name_list); $i++) {
    ...
    $result = mysqli_query($link, "SELECT text AS メッセージ , MAX(created_at)
    FROM direct_message
    WHERE (profile_id = " . $_COOKIE["id"] . " OR destination_profi-le_id = " . $_COOKIE["id"] . ")
    AND (profile_id = " . $partner_id . " OR desitnation_profile_id = " . $partner_id . ")
    GROUP BY id");
    $row = mysqli_fetch_assoc($result);
    //最後のメッセージ格納
    $name_list[$i]["メッセージ"] = $row;
}
mysqli_close($link);
require_once "./tpl/meassages.php";
