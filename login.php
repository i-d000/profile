...
if(isset($_POST['btn']) && $_POST['btn'] === 'login'){
    ...
    if(!empty('loginId') && !empty($_POST['pass'])){
        ...
        $getSql = "SELECT id AS id,password AS hashPass,salt AS solt,stretching AS stretch FROM profile WHERE position = '".$position."' AND year = '".$year."' AND season= '".$season."' AND personal_id = '".$personal."'";
        // 値取得
